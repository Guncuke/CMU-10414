...
void ReduceMax(const CudaArray& a, CudaArray* out, size_t reduce_size) {
  ...
  /// BEGIN SOLUTION
  CudaDims dim = CudaOneDim(out->size);
  ReduceMaxKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, out->size * reduce_size, reduce_size);
  /// END SOLUTION
}
...
void ReduceSum(const CudaArray& a, CudaArray* out, size_t reduce_size) {
  ...
  /// BEGIN SOLUTION
  CudaDims dim = CudaOneDim(out->size);
  ReduceSumKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, out->size * reduce_size, reduce_size);
  /// END SOLUTION
}
